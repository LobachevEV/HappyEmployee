...
const ChildComp: FunctionComponent<IEmployeeEditDialog> = (props) => {
  ...
  const handleEmploymentDateChange = (date: MaterialUiPickersDate) => handleChange({
    target: {
      name: "employmentDate",
      value: date?.toDate()
    }
  ...
  useEffect(() => {
    employee.employmentDate || handleChange({target: {name: "employmentDate", value: new Date()}});
